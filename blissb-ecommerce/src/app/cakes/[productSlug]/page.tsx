...
// Pre-renders every cake's product page at build time (ISR, same 60s
// revalidate window the underlying Strapi fetch already uses) instead of
// rendering every visit on demand.
export async function generateStaticParams() {
  const products = await getProductsByCategoryAsync("cakes");
...
export async function generateMetadata({
  ...
  if (!product || product.category !== "cakes") return {};
