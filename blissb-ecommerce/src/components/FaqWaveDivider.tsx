...
/**
 * Standalone copy of WaveDivider used only by the FAQ section.
 * WaveDivider itself hardcodes `relative` ahead of the passed className, which
 * can conflict with an `absolute` override — kept isolated here (with a proper
 * `cn()` merge) instead of changing the shared component every other section relies on.
 */
export function FaqWaveDivider({ direction = "bottom", color = "#fffff", className = "" }: FaqWaveDividerProps) {
  return (
